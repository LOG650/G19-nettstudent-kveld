...
Oppdatert: 2026-06-10 (Kapittel 3 (Teori) styrket etter muntlig veiledertilbakemelding: metodebegrunnelsene er løftet fram i hver underseksjon, ny rammeintro forklarer kontrasten tolkbar benchmark vs. fleksibelt ensemble og rammene prediksjon-vs-inferens og modellomfang, fremoverhenvisninger til kap. 5–9 er vevd inn, og ny §3.5 «Syntese» med Tabell 3.1 kartlegger teori → metode → analyse; innholdsfortegnelsen er utvidet og `rapport_final.pdf` (37 sider) regenerert. Samme dag tidligere: Rapportopprydding etter veiledertilbakemelding: interne `006 analysis`-mappereferanser er fjernet fra `rapport_final.md` (prosa i §5.3 og kap. 12, og vedleggstabell 12.1 viser nå rene filnavn i stedet for repo-stier), og forfatterne på forsiden er sortert alfabetisk etter etternavn (Bjerke, Brendehaug, James, Rånes); `rapport_final.pdf` regenerert. Samme dag tidligere: WBS 8.1 – nytt korrelasjonsvarmekart (Figur 8.3) lagt til i kap. 8 og koblet til §9.3 som empirisk belegg for støygulvet; analyseskript og artefakter i aktivitet 07, og `rapport_final.pdf` regenerert. Samme dag enda tidligere: §9.3 utvidet til en eksplisitt «Hvorfor presterer modellene så likt?»-del med parsimoni-poeng. Tidligere 2026-06-08: veiledertilbakemelding innarbeidet i ny revisjon.)
...
- [ ] WBS 8.1: Innarbeide veiledertilbakemelding og gjennomføre videre revisjon av `rapport_final.md`. `påbegynt 2026-06-08`
  ...
  - 2026-06-10: Nytt korrelasjonsvarmekart lagt til som empirisk belegg for støygulv-argumentet i §9.3. Nytt skript [start_wbs_korrelasjon.py](../006%20analysis/aktiviteter/07_eksplorativ_analyse_og_visualisering/start_wbs_korrelasjon.py) leser `06_datasplitt/X_train.csv` + `y_train.csv` (6 682 treningsrader) og produserer `fig_korrelasjon_numerisk.png`, `tab_korrelasjon_numerisk.csv` og `korrelasjon.md` i aktivitet 07. Varmekartet dekker de 9 numeriske variablene (`Sales` + `Discount` + 7 kalendervariabler); one-hot-kategorier er holdt utenfor. Hovedfunn: `Sales` har tilnærmet null korrelasjon med samtlige prediktorer (alle |r| ≤ 0,02), mens kalendervariablene `quarter`/`month`/`weekofyear` er sterkt innbyrdes korrelert (opptil 0,98) som mekanisk multikollinearitet. Figuren er satt inn som **Figur 8.3** sist i kap. 8 (Analyse) og henvist fra §9.3. `rapport_final.pdf` (35 sider) regenerert; Figur 8.3 verifisert på side 27.
  - 2026-06-10: Kapittel 3 (Teori) styrket etter muntlig veiledertilbakemelding (teorien skal begrunne hvorfor nettopp disse metodene er valgt, vise hvordan teori underbygger metodevalg og analyse, og gjøre videre kapitler mer logiske). Konkret: (1) hver underseksjon 3.1–3.4 åpner nå med en eksplisitt «hvorfor valgt»-setning; (2) ny rammeintro etablerer kontrasten tolkbar lineær benchmark vs. fleksibelt Random Forest-ensemble på bias–varians/tolkbarhet-aksen, samt rammene prediksjon-vs-inferens og modellomfang (ingen autoregressiv tidsrekke); (3) fremoverhenvisninger til §5.1–§5.3, kap. 6 (Tabell 6.1–6.2), kap. 7 (Tabell 7.3, Figur 7.2–7.3) og §8/§9.2/§9.3/§9.5 er vevd inn slik at teorien peker på hvor den brukes; (4) tre teorirammer lagt til (prediksjon vs. inferens, som begrunner fravær av formelle diagnosetester; modellomfang; hyperparametertuning/validering); (5) ny §3.5 «Syntese: teori, metode og analyse» med Tabell 3.1 som kartlegger hvert teoretisk grunnlag mot metodevalg og senere bruk i resultat/analyse/diskusjon. Innholdsfortegnelsen er utvidet med §3.5. Kun `rapport_final.md` endret; eksisterende ligninger (3.1–3.4) og øvrige numre er uendret. `rapport_final.pdf` regenerert via byggeskriptene i `005 report/build/` (pandoc 3.1.3 + lualatex), nå 37 sider (var 35).
