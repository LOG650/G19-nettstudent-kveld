...
- [x] G: Prognoser for 2025, `RMSE`/`MAPE`, modellsammenligning, analyse av viktige variabler og første tolkning av modellresultatene er nå dokumentert i WBS 5.1, 5.2, 5.3, 5.4 og 6.1 for lineær regresjon, Random Forest-baseline og tuned Random Forest. `tuned RF` fremstår samlet som anbefalt modell på 2025-data, vinner `RMSE` i de fleste måneder og segmenter, men `MAPE` varierer tydeligere mellom kvartal, rabatt, region og salgsnivå. `verifisert`
- [ ] H: Rapportskriving er påbegynt med korte metode- og modelleringsnotater om WBS 4.1, 4.2, 4.3, 4.4, 5.1, 5.2, 5.3, 5.4 og 6.1, men hoveddelen av rapportarbeidet er fortsatt ikke dokumentert i rapportmappen. `delvis verifisert`
...
- [x] Starte løpende rapportskriving parallelt med analysearbeidet, i tråd med arbeidsreglene i prosjektet. `påbegynt 2026-03-30, utvidet 2026-04-02 med modellnotater for WBS 4.1, 4.2, 4.3, 4.4 og 5.1, og utvidet 2026-04-07 med korte notater for WBS 5.2, 5.3, 5.4 og 6.1`
- [ ] Oppdatere denne statuslisten etter ukentlige statusmøter og ved milepæler.

## Anbefalt kort prosjektstatus

Prosjektet fremstår per 2026-04-07 som værende ferdig med planfasen, med dataforståelse, datasettdokumentasjon, remappet datarensing, feature engineering, datasplitt, eksplorativ analyse, lineær regresjon for WBS 4.1, Random Forest for WBS 4.2, felles verifisering av treningsgrunnlaget i WBS 4.3, parameterjustering av Random Forest i WBS 4.4, genererte 2025-prognoser i WBS 5.1, beregnede 2025-`RMSE`/`MAPE` i WBS 5.2, dokumentert modellsammenligning i WBS 5.3, analysert variabelprioritering i WBS 5.4 og tolket modellmønstre i WBS 6.1. `tuned RF` fremstår samlet som beste modell på 2025-data, men WBS 6.1 viser at prosentfeilen varierer tydeligere mellom kvartal, rabatt, region og salgsnivå enn absoluttfeilen. Neste kritiske behov er å diskutere styrker og svakheter, vurdere praktisk nytte og bygge disse funnene tydelig inn i rapporten.
